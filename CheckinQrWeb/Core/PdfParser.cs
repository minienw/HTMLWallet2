...
public class PdfParser
{
    ...
    /// <returns></returns>
    public unsafe PdfParser(Stream buffer)
    {
        using var ms1 = GetMemoryStream(buffer);
        fpdfview.FPDF_InitLibrary();
        fixed (byte* ptr = ms1.GetBuffer())
            _Doc = fpdfview.FPDF_LoadMemDocument((IntPtr)ptr, (int)ms1.Length, "");
    }
