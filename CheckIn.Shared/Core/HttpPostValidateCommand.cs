...
public class HttpPostValidateCommand 
{
    ...
    public async Task<HttpPostValidateResult> Execute(HttpPostValidateArgs args)
    {
        ...
        var plainText = Encoding.UTF8.GetBytes(args.DccQrCode);
        var encryptedDcc = Crypto.EncryptAesCbc(plainText, secretKey, args.IV);
        ...
        var digest = Crypto.Sign(plainText, args.WalletPrivateKey);
